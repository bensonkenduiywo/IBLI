...
regfun <- function(x, y, triggers, main="") {
  ...
  plot(y~x, pch=16, main=main, cex=0.8)
  ...
  legend("topright", c("lm", "lm0", "lm5", "seg"), col=c("magenta", "blue", "green", "red"), lwd=c(3,3,3,2), lty=c(2,3,4,1), title="Model", bty="n")
  test
}
...
trigs <- round(quantile(mort, seq(0.6, 0.9, 0.1)), 1)
trigs
...
```{r p3}
x11()
par(mfrow=c(1, 2), mar=c(12, 3.6, 1.5, 1)) #c(bottom, left, top, right)
res <- list()
res$noaa <- regfun(dff$znoaa, dff$mortality_rate, trigs, "Z-NOAA")
res$lognoaa <- regfun(dff$zlnoaa, dff$mortality_rate, trigs, "Z-log NOAA")
```
...
x11()
par(mfrow=c(1, 2), mar=c(12, 3.6, 1.5, 1)) #c(bottom, left, top, right)
res$rain <- regfun(dff$zrain, dff$mortality_rate, trigs, "Z-rain")
res$lograin <- regfun(dff$zlrain, dff$mortality_rate, trigs, "Z-log rain")
```
...
x11()
par(mfrow=c(1, 2), mar=c(12, 3.6, 1.5, 1)) #c(bottom, left, top, right)
res$modis <- regfun(dff$zmodis, dff$mortality_rate, trigs, "Z-MODIS")
res$logmodis <- regfun(dff$zlmodis, dff$mortality_rate, trigs, "Z-log MODIS")
```
...
These values can be used to compute a relative mqs score `rmqs`. That could be an interesting measure. 
