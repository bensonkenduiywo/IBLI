...
To run this you need to install the latest versions of terra, agro, and agrodata `remotes::install_github("reagro/agrodata")`.
...
```{r}
res <- list()
res$noaa <- regfun(dff$znoaa, dff$mortality_rate, trigs, "NOAA")
```

Log transformed NOAA NDVI.

```{r}
res$lognoaa <- regfun(dff$zlnoaa, dff$mortality_rate, trigs, "log NOAA")
```

CHIRPS rainfall.

```{r}
res$rain <- regfun(dff$zrain, dff$mortality_rate, trigs, "rain")
```

Log transformed CHIRPS rainfall.

```{r}
res$lograin <- regfun(dff$zlrain, dff$mortality_rate, trigs, "log rain")
```

MODIS NDVI.

```{r}
res$modis <- regfun(dff$zmodis, dff$mortality_rate, trigs, "modis")
```

Log transformed MODIS NDVI.

```{r}
res$logmodis <- regfun(dff$zlmodis, dff$mortality_rate, trigs, "log modis")
```
...
These values can be used to compute a relative mqs score `rmqs`. That could be an interesging measure. 

```{r}
xx <- x
xx[,7:ncol(xx)] <- xx[,7:ncol(xx)] / rep(pf, each=nrow(xx))
