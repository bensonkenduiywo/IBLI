...
raster::spplot(lr, "mortality_rate", col.regions=cols,
                    ...
                    main="LRLD mean mortality")
x11()
raster::spplot(sr, "mortality_rate", col.regions=cols,
                    sp.layout = make.NUMB.sp.label(sr), 
                    cex = 0.5,
                    bg = "white", col="light grey", border="light grey",
                    main="SRSD mean mortality")
...
regfun <- function(x, y, main="") {
  ...
  df0 <- df[df$x<0,]
  df5 <- df[df$x < -0.5,]
  test <- data.frame(matrix(nrow=5,ncol=6))
  colnames(test) <- c("group", "model", "r2", "RMSE", "intercept", "slope")
  test$group <- main
  test$model <- c("lm", "lm0", "lm5", "seg", "loess")
  ...
  e <- predict(mz)
  test[5,3:4] <- c(Rsq(df$y, e), rmse(df$y - e))
  test
}
...
Get rid of loess, and save for use in next steps.
...
Mortality very high in LOIYANGALANI, and also high in LOGOLOGO and SAGANTE
Very low in KALACHA, TURBI, BUBISA (they should not take insurance)
