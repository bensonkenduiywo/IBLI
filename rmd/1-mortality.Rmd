...
We will determine the effect rainfall data transformation on mortality predictions based on R$^2$, Mean Absolute Prediction Error (MAPE) and Root Mean Square Error (RMSE). For instance, a MAPE of (1) < 10 indicates highly accurate prediction, while (2) 10--0 good prediction, 3) 20 -- reasonable prediction, and (4) > 50 inaccurate.
...
get_benefit <- function(df, rho){
  #Define trigger
  data <- na.omit(df)
  trig <- quantile(data$predicted_mortality, 0.80, na.rm=T)
  #Compute payouts
  data$payouts <- pmax(0, data$predicted_mortality - trig)/100 * 1000
...
regfun <- function(x, y, main="") {
  ...
  df0 <- df[df$x<0,]
  df5 <- df[df$x < -0.5,]
  test <- data.frame(matrix(nrow=4,ncol=7))
  colnames(test) <- c("group", "model", "r2", "RMSE", "intercept", "slope","Benefit($)")
  test$group <- main
  ...
  test[1,7] <- get_benefit(df, rho)
  test[1,3:4] <- c(Rsq(df$y, e), rmse(df$y - e))
  test[1,5:6] <- coefficients(ml)
  
  e <- predict(ml2)
  df$predicted_mortality <- NA
  df$predicted_mortality[df$x<0] <- e
  test[2,7] <- get_benefit(df, rho)
  test[2,3:4] <- c(Rsq(df0$y, e), rmse(df0$y - e))
  test[2,5:6] <- coefficients(ml2)
  
  e <- predict(ml5)
  df$predicted_mortality <- NA
  df$predicted_mortality[df$x < -0.5] <- e
  test[3,7] <- get_benefit(df, rho)
  test[3,3:4] <- c(Rsq(df5$y, e), rmse(df5$y - e))
  test[3,5:6] <- coefficients(ml5)
  
  e <- predict(sm)
  df$predicted_mortality <- e
  test[4,7] <- get_benefit(df, rho)
  test[4,3:4] <- c(Rsq(df$y, e), rmse(df$y - e))
  ...
  legend("topright", c("Linear", "Piecewise x < 0", "Piecewise x < -0.5", "Segmented"), col=c("magenta", "blue", "green", "red"), lwd=c(3,3,3,2), lty=c(2,3,4,1), title="Regression model")
  test
}
...
```{r}
res <- list()
res$noaa <- regfun(dff$znoaa, dff$mortality_rate, "NOAA")
```

Log transformed NOAA NDVI.

```{r}
res$lognoaa <- regfun(dff$zlnoaa, dff$mortality_rate, "log NOAA")
```

CHIRPS rainfall.

```{r}
res$rain <- regfun(dff$zrain, dff$mortality_rate, "rain")
```

Log transformed CHIRPS rainfall.

```{r}
res$lograin <- regfun(dff$zlrain, dff$mortality_rate, "log rain")
```

MODIS NDVI.

```{r}
res$modis <- regfun(dff$zmodis, dff$mortality_rate, "modis")
```

Log transformed MODIS NDVI.

```{r}
res$logmodis <- regfun(dff$zlmodis, dff$mortality_rate, "log modis")
```
...
Save model for use in next steps.
...
quantile(mort, c(0.75, 0.8))

aggregate(dff[, "mortality_rate", drop=FALSE], dff[,"SUBLOCATION", drop=FALSE], function(i) quantile(i, c(0.75, 0.8), na.rm=T))
```

Mortality very high in LOIYANGALANI, LOGOLOGO and SAGANTE. In contrast it is Very low in KALACHA, TURBI, BUBISA (they should not take insurance). This is supported by observed mortalities as displayed in spatial variation maps previously. 
