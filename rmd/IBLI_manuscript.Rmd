...
#rainfall+NDVI
m_c <- loess(mortality_rate ~ zndvi + zrain , data=dd)
p <- predict(m_c, dd)
plot(dd$mortality_rate, p, xlab="Observed mortality", ylab="Predicted mortality", main="NDVI & rainfal model", pch=16, cex.axis=1.2, cex.lab=1.2, cex=0.7)
abline(0,1, lwd=2, col="red")
cat("NDVI & Rainfall model Residual Standard Error: ", m_c$s, "\n")
```
...
par(mai=c(1,2.5,1,1))
boxplot(df$rain_mortality ~ df$year, ylab="Predicted mortality rate", xlab="Year", cex= 0.5, cex.lab=1.2, main="Rainfall model")

#NDVI predictions
df$ndvi_mortality <- predict(m_n, df)
par(mai=c(1,2.5,1,1))
boxplot(df$ndvi_mortality ~ df$year, ylab="Predicted mortality rate", xlab="Year", cex= 0.5, cex.lab=1.2, main="NDVI model")
...
Assume we design an insurance contract that pays every five years. Let define a trigger using 80th percentile value of rainfall and ndvi mortality predictions. For instance, when mortality is above 10%.
...
The trigger first occurs at this *zNDVI* and *zrainfall*.

```{r m9}
df <- na.omit(df)
#zNDVI
tndvi <- max(df$zndvi[df$rain_mortality > r_trig])
round(tndvi, 2)
#zrainfall
train <- max(df$zrain[df$ndvi_mortality > n_trig])
round(train, 2)
```

The payout — in units of the monetary value of TLU — can be computed as simply the percentage difference between predicted mortality in TLU and the trigger. Those below the trigger get no payout. We assume that 1 TLU has a monetary equivalence of 1000$.
...
Plots for payouts and triggers.
...
plot(epay(zndvi)~zndvi, type="l", data=df)
```
