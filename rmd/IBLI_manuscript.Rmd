...
So get the CE function.

```{r e1}
data <- df[df$year >=2008,]
temp <- na.omit(d[, c("sublocation", "season", "year", "mortality_rate")])
data <- merge(data, temp, by=c("sublocation", "season", "year"))
```

Compute mortality losses as the difference between the observed mortality (mortality_rate) and the trigger we adopted previously. 

```{r e2}
trigger <- round(mean(r_trig,n_trig), 2)
data$loss <- pmax(0, data$mortality_rate - trigger) * 1000
