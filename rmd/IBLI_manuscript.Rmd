...
```{r m9}
x11()
plot(ndvi_mortality[order(zndvi)]~zndvi[order(zndvi)], data=df, xlab="zNDVI", ylab="Predicted mortality", pch=16, type="l", lwd=1.2)
...
```{r m10}
df$r_pay <- pmax(0, df$rain_mortality - r_trig) * 1000
df$n_pay <- pmax(0, df$ndvi_mortality - n_trig) * 1000
```
...
x11()
plot(r_pay[order(r_pay)]~zrain[order(r_pay)], data=df, ylab="Payouts (USD)", xlab="z-rainfall", cex.lab=1.2, cex.axis=1.2, pch=16, cex=0.7, type="l", lwd=1.2)
...
x11()
plot(rhos, n_ce$ce_base, type="l", ylab= "Pay per TLU (USD)", xlab="CRRA", cex=2)
...
lines(rhos, mqs_r$mqs_no, col="black")
lines(rhos, mqs_r$mqs, col="blue")
abline(h=0, col="black", cex=0.6)
legend("topleft", c("NDVI 25% Marked-up", "NDVI No mark up", "Rainfall No mark up", "Rainfall 25% Marked-up"), lty=1, col=c("red", "green", "black", "blue"), title = "Insurance type", bty = "n")
