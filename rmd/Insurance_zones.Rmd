...
for(z in 1:length(zones)){
  set.seed(99)
  r_kmn <- kmeans(df[,-1], centers = zones[z], iter.max = 500, nstart = 2, algorithm="Lloyd")
  # kmeans returns an object of class "kmeans"
  subloc$Zones <- r_kmn$cluster
  df2 <- 0
...
quality <- data.frame(Zone=zones,ndvi_R2=r_n,rain_R2=r_r, ndvi_RMSE=e_n,rain_RMSE=e_r,ndvi_mape=mape_n,rain_mape=mape_r)
quality
...
x11()
plot(zones, a[,1], type="l", col="red", ylab= "MQS (insurance benefit per TLU in $)", xlab="Sub-location clusters", cex=2, ylim=c(0, 30))
