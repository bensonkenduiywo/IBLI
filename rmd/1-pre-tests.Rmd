...
Here we test the effect of shortening the rainfall and NOAA AVHRR data to MODIS time series length on R$^2$.

```{r t1}
#Long time period
df <- merge(p, n, by = c("SUBLOCATION","year", "season"))
df3 <- ungroup(mutate(group_by(df, SUBLOCATION, season), znoaa=zscore(noaa), zrain=zscore(rainfall)))
df3 <- merge(df3, amort, by=c("year", "season", "SUBLOCATION"))
#rainfall predictions from long time series (1982--2019)
m_rz <- loess(mortality_rate ~ zrain , data=df3, control=loess.control(surface="direct"))
e <- predict(m_rz, df3)
R_square(df3$mortality_rate, e)
#rainfall predictions from shortened time series(2001--2019)
df4 <- df[df$year > 2000,]
