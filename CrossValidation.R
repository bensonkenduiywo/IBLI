...
#lno <- aggregate(lno[,c("R2", "RMSE"), drop=FALSE], lno[, c("group","model"), drop=FALSE], mean, na.rm=TRUE)
mo <- Reduce(function(...) merge(..., all=T), mo)
#mo <- aggregate(mo[,c("R2", "RMSE"), drop=FALSE], mo[, c("group","model"), drop=FALSE], mean, na.rm=TRUE)
lmo <- Reduce(function(...) merge(..., all=T), lmo)
#lmo <- aggregate(lmo[,c("R2", "RMSE"), drop=FALSE], lmo[, c("group","model"), drop=FALSE], mean, na.rm=TRUE)
rn <- Reduce(function(...) merge(..., all=T), rn)
#rn <- aggregate(rn[,c("R2", "RMSE"), drop=FALSE], rn[, c("group","model"), drop=FALSE], mean, na.rm=TRUE)
